...
export class CampaignTabComponent implements OnInit {
  ...
  ngAfterViewInit() {
    $('#camp_grid').on("click", ".serviceGroup", function () {
      window["serviceDetails"]($(this).attr("data-id"));
    });

    $('#camp_grid').on("click", ".manageuserrights", function () {
  ...
  public GetUsersList() {
    ...
    if ($.fn.DataTable.isDataTable('#camp_grid')) {
      $('#camp_grid').dataTable().fnDestroy();
      $('#camp_grid').empty();
    }
