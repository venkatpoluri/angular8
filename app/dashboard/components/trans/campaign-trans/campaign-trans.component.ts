...
@Component({
  selector: 'app-campaign-trans',
  templateUrl: './campaign-trans.component.html',
  styleUrls: ['./campaign-trans.component.css'],
  animations: [
    trigger('openClose', [
      state('open', style({
        display: 'block',
...
export class CampaignTransComponent implements OnInit, AfterViewInit {

  public dateTime = new moment();
  displayedColumns: string[] = ["gtrid", "campaign_id", "campaign_title_es",  "originating_timestamp", "cgw_gtrid", "charging_amount", "product_description",
    "description", "servicename_es", "service_type_es", "servicegroup_es", "delivery_type", "cgw_originating_timestamp", "coupon_code", "actions"];
  ...
  gettransactions() {
    ...
    this.spinner.show();
    this.ccapi.postData('subscriberdata/GetCampaignTransactions', requesrParams).subscribe((response: any) => {
