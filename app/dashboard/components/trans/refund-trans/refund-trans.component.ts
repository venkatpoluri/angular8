...
//import messageTrans from '../refund-trans/refund-trans.json';
...
@Component({
  selector: 'app-refund-trans',
  templateUrl: './refund-trans.component.html',
  styleUrls: ['./refund-trans.component.css'],
  animations: [
    trigger('openClose', [
      state('open', style({
        display: 'block',
...
export class RefundTransComponent implements OnInit, AfterViewInit {

  public dateTime = new moment();
  displayedColumns: string[] = ["servicename_es", "charged_on", "charged_price", "charge_requested_channel", "refund_date", "status",
    "refund_type", "refund_amount", "refund_reason_code", "refund_description", "error_description", "agent_name", "actions"];
  ...
  gettransactions() {
    ...
    this.spinner.show();
    this.ccapi.postData('subscriberdata/GetRefundTransactions', requesrParams).subscribe((response: any) => {
