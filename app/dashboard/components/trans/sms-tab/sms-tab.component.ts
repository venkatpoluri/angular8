...
@Component({
  ...
  styleUrls: ['./sms-tab.component.css']
})
export class SmsTabComponent implements OnInit {
  Date = new Date();
  ...
  ngAfterViewInit() {
    $('#sms_grid').on("click", ".serviceGroup", function () {
      window["serviceDetails"]($(this).attr("data-id"));
    });

    $('#sms_grid').on("click", ".manageuserrights", function () {
  ...
  public GetUsersList() {
    ...
    if ($.fn.DataTable.isDataTable('#sms_grid')) {
      $('#sms_grid').dataTable().fnDestroy();
      $('#sms_grid').empty();
    }
    ...
    try {
      ...
      var tbl = $('#sms_grid').DataTable({
        ...
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.spinner.show();
          this.ccapiService.postData('subscriberdata/GetSMSTransactions', requesrParams).subscribe((resp: any) => {
            this.spinner.hide();
