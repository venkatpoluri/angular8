...
@Component({
  selector: 'app-refunds-tab',
  templateUrl: './refunds-tab.component.html',
  animations: [
    trigger('openClose', [
      state('open', style({
        display: 'block',
  ...
  styleUrls: ['./refunds-tab.component.css']
})
export class RefundsTabComponent implements OnInit {
  Date = new Date();
  ...
  ngAfterViewInit() {
    $('#refunds_grid').on("click", ".serviceGroup", function () {
      window["serviceDetails"]($(this).attr("data-id"));
    });

    $('#refunds_grid').on("click", ".manageuserrights", function () {
  ...
  public GetUsersList() {
    ...
    if ($.fn.DataTable.isDataTable('#refunds_grid')) {
      $('#refunds_grid').dataTable().fnDestroy();
      $('#refunds_grid').empty();
    }
    ...
    try {
      var columns = [
        ...
        {
          "data": "message", title: "DIAL STRING", "className": "td-break", "width": "300px", "render": function (data, type, row) {
        ...
        {
          "data": "originating_timestamp", title: "RECEIVED ON", "width": "150px", "render": function (data) {
            return encodeURI(String(data));
          }
        },
        {
        ...
        {
          "data": "gtrid", title: "ACTIONS", "width": "50px", "render": function (data, type, row, meta) {
            if (type === 'display') {
      ...
      try {
        $('#refunds_grid').destroy();
      }
      catch (e) { }
      var tbl = $('#refunds_grid').DataTable({
        destroy: true,
        // bLengthChange:false,
        scrollX: true,
        scrollY: "400px",
        ...
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.spinner.show();
          this.ccapiService.postData('subscriberdata/GetCCAREUSSDTransactions', requesrParams).subscribe((resp: any) => {
            this.spinner.hide();
      ...
      $('#refunds_grid').on('page.dt', function () {
