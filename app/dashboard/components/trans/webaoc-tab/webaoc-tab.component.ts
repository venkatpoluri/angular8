...
@Component({
  selector: 'app-webaoc-tab',
  templateUrl: './webaoc-tab.component.html',
  animations: [
    trigger('openClose', [
      state('open', style({
        display: 'block',
  ...
  styleUrls: ['./webaoc-tab.component.css']
})
export class WebaocTabComponent implements OnInit {
  Date = new Date();
  ...
  ngAfterViewInit() {
    $('#webaoc_grid').on("click", ".serviceGroup", function () {
      window["serviceDetails"]($(this).attr("data-id"));
    });

    $('#webaoc_grid').on("click", ".manageuserrights", function () {
  ...
  public GetUsersList() {
    ...
    if ($.fn.DataTable.isDataTable('#webaoc_grid')) {
      $('#webaoc_grid').dataTable().fnDestroy();
      $('#webaoc_grid').empty();
    }
    ...
    try {
      ...
      $('#webaoc_grid').on('page.dt', function () {
