...
@Component({
  selector: 'app-ussd-tab',
  templateUrl: './ussd-tab.component.html',
  animations: [
    trigger('openClose', [
      state('open', style({
        display: 'block',
  ...
  styleUrls: ['./ussd-tab.component.css']
})
export class UssdTabComponent implements OnInit {
  Date = new Date();
  ...
  ngAfterViewInit() {
    $('#ussd_grid').on("click", ".serviceGroup", function () {
      window["serviceDetails"]($(this).attr("data-id"));
    });

    $('#ussd_grid').on("click", ".manageuserrights", function () {
  ...
  public GetUsersList() {
    ...
    if ($.fn.DataTable.isDataTable('#ussd_grid')) {
      $('#ussd_grid').dataTable().fnDestroy();
      $('#ussd_grid').empty();
    }
