...
@Component({
  selector: 'app-hsdp-tab',
  templateUrl: './hsdp-tab.component.html',
  animations: [
    trigger('openClose', [
      state('open', style({
        display: 'block',
  ...
  styleUrls: ['./hsdp-tab.component.css']
})
export class HsdpTabComponent implements OnInit {
  Date = new Date();
  ...
  ngAfterViewInit() {
    $('#hsdp_grid').on("click", ".serviceGroup", function () {
      window["serviceDetails"]($(this).attr("data-id"));
    });

    $('#hsdp_grid').on("click", ".manageuserrights", function () {
  ...
  public GetUsersList() {
    ...
    if ($.fn.DataTable.isDataTable('#hsdp_grid')) {
      $('#hsdp_grid').dataTable().fnDestroy();
      $('#hsdp_grid').empty();
    }
